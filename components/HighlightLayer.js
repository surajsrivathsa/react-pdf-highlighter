...
export function HighlightLayer({ highlightsByPage, scaledPositionToViewport, pageNumber, scrolledToHighlightId, highlightTransform, tip, hideTipAndSelection, viewer, screenshot, showTip, setState, currentPage, realPageNumber, pageSize, }) {
    ...
    return (React.createElement("div", null, currentHighlights.map((_a, index) => {
        ...
        return highlightTransform(viewportHighlight, index, (highlight, callback) => {
            ...
            const viewport = viewer.getPageView((cutPageNumber || rect.pageNumber || parseInt(pageNumber)) - 1).viewport;
